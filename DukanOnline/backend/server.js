...
app.get("/", (req, res) => res.json({ message: "DukanOnline API is running" }));
...
io.on("connection", (socket) => {
  ...
  socket.emit("chat:system", {
    id: `system-${Date.now()}`,
    text: `Hi ${socket.user.name}! Welcome to DukanOnline support. How can we help you?`,
    sender: "server",
...
function getServerResponse(message) {
  ...
  if (/hello|hi|hey/.test(text)) {
    return "Hello! 👋 How can I help you with your DukanOnline order?";
  }
